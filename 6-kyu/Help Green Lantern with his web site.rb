...
def yellow_be_gone(color)
  return TC[color.downcase] ? TC[color.downcase] : color unless color.include?('#')
  dc = color[1..-1].chars.each_slice(2).map{|a| a.join.to_i(16)}
  dc = [dc.sort[0], dc.sort[2], dc.sort[1]] if dc[2] < dc[1] && dc[2] < dc[0]
  res = dc.map{|n| (n.to_s(16).size == 1 ? '0' : '') + n.to_s(16)}.join
  '#' + (color.downcase == color ? res : res.upcase)
end
