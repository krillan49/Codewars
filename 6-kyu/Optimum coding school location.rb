...
def optimum_location(students, locations)
  locations.each do |hh|
    arr = []
    students.each{|x, y| arr << (x - hh['x']).abs + (y - hh['y']).abs}
    hh['distanse'] = arr
  end
  res = locations.sort_by{|hh| hh['distanse'].sum}[0]
  "The best location is number #{res['id']} with the coordinates x = #{res['x']} and y = #{res['y']}"
end
