...
def find_word(num_let, max_ssw)
  listlet = $word_list.select{|w| w.size == num_let}
  return nil if listlet == []
  listres = listlet.map.with_index{|w, i| [w, w.chars.uniq.map{|c| c.ord * w.count(c)}.sum, i]}.select{|a| a[1] <= max_ssw}
  return nil if listres == []
  listres.sort_by{|a| [a[1], a[2]]}[-1][0]
end
