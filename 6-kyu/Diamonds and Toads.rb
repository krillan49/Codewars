...
def diamonds_and_toads(sent, fairy)
  if fairy == :good
    {ruby: sent.tr('^R','').size * 2 + sent.tr('^r','').size, crystal: sent.tr('^C','').size * 2 + sent.tr('^c','').size }
  else
    {python: sent.tr('^P','').size * 2 + sent.tr('^p','').size, squirrel: sent.tr('^S','').size * 2 + sent.tr('^s','').size }
  end
  .select{|k, v| v != 0}
end
