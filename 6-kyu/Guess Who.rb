...
class GuessWho
  ...
  def guess(guess)
    @counter += 1
    return ["Correct! in #{@counter} turns"] if guess == @character
    if @characteristic.map{|a| a[1]}.flatten.uniq.include?(guess)
      tipes(guess)
    else
      @characteristic = @characteristic.reject{|a| a[0] == guess}
      @characteristic.map(&:first)
    end
  end
  ...
  def tipes(guess)
    if @characteristic.select{|a| a[0] == @character}.map{|a| a[1]}.flatten.include?(guess)
      @characteristic = @characteristic.select{|a| a[1].include?(guess)}
    else
      @characteristic = @characteristic.reject{|a| a[1].include?(guess)}
    end
    @characteristic.map(&:first)
  end
