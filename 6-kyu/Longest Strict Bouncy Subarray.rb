...
def longest_bouncy_list(lst)
  ar = lst.each_cons(3)
          .map{|a, b, c| b > [a, c].max || b < [a, c].min ? [a, b, c] : nil}
          .slice_when{|a, b| !a || !b}
          .max_by(&:size)
  r = ar.map.with_index{|a, i| i == ar.size - 1 ? a : a[0]}.flatten
  r = lst.each_cons(2).find{|a, b| a != b} if r == [nil]
  r || [lst[0]]
end
