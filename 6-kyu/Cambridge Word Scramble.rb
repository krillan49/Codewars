...
def mix_words(str)
  return 'undefined' if str.class != String
  a = str.chars.slice_when{|a, b| (/[a-z]/i === a && /[^a-z]/i === b) || (/[a-z]/i === b && /[^a-z]/i === a)}.to_a
  return str if a.all?{|ar| ar.size < 4 || ar.all?{|c| /[^a-z]/i === c}}
  loop do
    a = a.map{|a| (a.size > 3 && a.all?{|c| /[a-z]/i === c}) ? [a[0]] + a[1..-2].shuffle! + [a[-1]] : a}
    return a.join if a.join != str
  end
end
