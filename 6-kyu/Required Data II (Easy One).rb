...
def given_nth_value(arr, k, str)
  return 'Incorrect value for k' if k.class != Integer or k < 1
  return "Valid entries: 'max' or 'min'" if str.class != String or !['max', 'min'].include?(str.downcase)
  return 'No values in the array' if arr.empty?
  return 'Invalid entry list' if arr.any?{|e| e.class != Integer}
  (str.downcase == 'max' ? arr.uniq.sort.reverse[k-1] : arr.uniq.sort[k-1]) || 'No way'
end
