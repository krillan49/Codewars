...
#
# Preloaded
# The Student class has been preloaded with the constructor accepting two parameters a name and marks which should be an array of numbers.
...
def calculate_improved(students)
  students.each do |hh|
    hh['marks'] = hh['marks'].map{|e| e == nil ? 0 : e}
    hh['improvement'] = hh['marks'][0] == 0 ? 0 : ((hh['marks'][-1] - hh['marks'][0]) * 100.0 / hh['marks'][0] ).round
    hh.delete('marks')
  end
  .sort_by{|hh| [-hh['improvement'], hh['name']]}
end
