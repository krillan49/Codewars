...
def distribute(nod, work)
  a, s = work / nod, work % nod
  f = Array.new(s * (a + 1), nil).map.with_index{|e, i| i}
  l = Array.new((nod - s) * a, nil).map.with_index(f.size){|e, i| i}
  ff = f.each_slice(a + 1).to_a
  l == [] ? ff + [[]] * (nod - ff.size) : ff + l.each_slice(a).to_a
end
