...
def avg_diags(m)
  d1 = Matrix[*m].each(:diagonal).select.with_index{|e, i| i.odd? && e >= 0}
  d2 = Matrix[*m.map(&:reverse)].each(:diagonal).to_a.reverse.select.with_index{|e, i| i.even? && e < 0}
  [d1.empty? ? -1 : (d1.sum.to_f / d1.size).round, d2.empty? ? -1 : (d2.sum.to_f / d2.size).abs.round]
end
