...
def add_check_digit(num)
  ind = [2, 3, 4, 5, 6, 7] * (num.size / 6 + 1)
  r = num.to_i.digits.zip(ind).reject{|a| a.include?(nil)}.map{|n, i| n * i}.sum % 11
  r == 0 ? num + '0' : r == 1 ? num + 'X' : num + (11 - r).to_s
end
