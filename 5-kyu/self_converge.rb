...
def self_converge(num)
  return -1 if num.digits.uniq == 1
  res = []
  size = num.to_s.size
  loop do
    num = num.digits.sort.reverse.join.to_i - num.digits.sort.join.to_i
    num = (num.to_s + '0' * (size - num.to_s.size)).to_i if num.to_s.size < size
    return res.size + 1 if res.include?(num)
    res << num
  end
end
