...
COL = {black: '0', brown: '1', red: '2', orange: '3', yellow: '4', green: '5', blue: '6', violet: '7', gray: '8', white: '9'}

def encode_resistor_colors(str)
  n = str.split[0].chars.slice_when{|a, b| /[a-z]/i === b}.map(&:join)
  .map{|e| e == 'k' ? 1000 : e == 'M' ? 1000000 : e.to_f}.inject(:*).round.to_s
  if n.to_i < 100
    n.chars.map{|e| COL.key(e)}.join(' ') + ' black gold'
  else
    n[0..1].chars.map{|e| COL.key(e)}.join(' ') + " #{COL.key(n[2..-1].size.to_s)}" + ' gold'
  end
end
