...
AN = [7]
(2..2000000).each{|n| AN << AN[-1] + n.gcd(AN[-1])}
GN = AN.each_cons(2).map{|a, b| b - a}.unshift(1)
PN = GN.reject{|e| e == 1}
ANO = AN.map.with_index{|e, i| GN[i] != 1 ? AN[i] / i : nil}.compact

def an(n) AN[n-1] end
def gn(n) GN[0, n] end
def pn(n) PN[0, n] end
def an_over(n) ANO[0, n] end

def count_ones(n) GN[0, n].count(1) end
def max_pn(n) PN.uniq[0, n].max end
def an_over_average(n) 3 end
