...
def change(s, pg, v)
  hh = s.split("\n").select{|w| w.include?('Phone') or w.include?('Version')}.map{|w| w.split(':').map(&:strip)}.to_h
  return 'ERROR: VERSION or PHONE' if !(hh['Phone']=~/\+1-\d{3}-\d{3}-\d{4}\z/) or !(hh['Version']=~/\A\d{1,}\.\d{1,}\z/)
  hh['Version']=v if hh['Version']!='2.0'
  "Program: #{pg} Author: g964 Phone: +1-503-555-0090 Date: 2019-01-01 Version: #{hh['Version']}"
end
