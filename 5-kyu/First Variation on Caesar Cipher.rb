...
def m_s_helper(m, k=0)
  m > 90 + k ? ((m - 65 - k) % 26 + 65 + k).chr : m.chr
end

def d_s_helper(m, k=0)
  r = ((65 + k - m) / 26.0).ceil
  m < 65 + k ? (m + 26 * r).chr : m.chr
end

def movingShift(s, shift)
  a = s.chars.map.with_index do |c, i|
    m = c.ord + shift + i
    /[A-Z]/ === c ? m_s_helper(m) : /[a-z]/ === c ? m_s_helper(m, 32) : c
  end
  arr, n, size = [], (a.size / 5.0).ceil, a.size
  5.times do
    if size >= n
      arr << [n]
      size -= n
  ...
  arr.map{|(x)| a.shift(x)}.map(&:join)
end

def demovingShift(arr, shift)
  arr.join.chars.map.with_index do |c, i|
    m = c.ord - shift - i
    /[A-Z]/ === c ? d_s_helper(m) : /[a-z]/ === c ? d_s_helper(m, 32) : c
  end.join
end
