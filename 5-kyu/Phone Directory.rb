...
def phone(strng, num)
  arr = strng.split("\n").select{|s| s.gsub(/[^0-9-]/, ' ').split.include?(num)}
  return "Error => Not found: #{num}" if arr.empty?
  return "Error => Too many people: #{num}" if arr.size > 1
  str = arr.join
  name = str.scan(/<.*>/).join
  str = str.gsub(/#{name}|#{num}/, '').tr('_', ' ').gsub(/[^a-z0-9. -]/i, '').split.join(' ')
  "Phone => #{num}, Name => #{name[1..-2]}, Address => #{str}"
end
