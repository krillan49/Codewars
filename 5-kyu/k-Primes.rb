...
def count_Kprimes(k, start, nd)
  (start..nd).select{|n| n != 0 && n.prime_division.sum(&:last) == k}
end

def puzzle(s)
  arr = (1..s).to_a
  a = arr.select(&:prime?)
  b = (arr - a).select{|n| n.prime_division.sum(&:last) == 3}
  c = (arr - a - b).select{|n| n.prime_division.sum(&:last) == 7}
  c.product(b, a).count{|ar| ar.sum == s}
end
