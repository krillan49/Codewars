...
def flap_display(lines, rotors)
  r = rotors.map{|a| a.map.with_index{|n, i| a[0..i].sum}}
  l = lines.map(&:chars)
  l.zip(r).map{|a, b| a.zip(b).map{|ar| $ALPHABET.index(ar[0]) + ar[1]}
  .map{|n| n < $ALPHABET.size ? $ALPHABET[n] : $ALPHABET[n % $ALPHABET.size]}.join}
end
